...
class _state(param.Parameterized):
    ...
    def snapshot(
        ...
        *extra_roots: t.Any,
        ...
        """
        Collect a state snapshot of all components that have an explicit
        ``snapshot_key`` parameter set, plus the current URL location.
        ...
        *extra_roots : Viewable
        ...
        -------
        dict
            A serializable snapshot dictionary.

        See Also
        --------
        state.restore_snapshot : apply a snapshot produced by this method.
        """
        from .snapshot import collect_state
        return collect_state(root, *extra_roots)
    ...
    def restore_snapshot(
        ...
        *extra_roots: t.Any,
        ...
        """
        Apply a snapshot previously produced by :meth:`state.snapshot`.

        Parameters
        ----------
        snapshot : dict
            The snapshot dictionary to apply.
        root : Viewable or iterable of Viewable, optional
        ...
        *extra_roots : Viewable
        ...
        -------
        dict
            ``{"applied": [...], "failed": [...]}`` listing the
            snapshot_keys that were or were not restored.
        """
        from .snapshot import apply_state
        return apply_state(snapshot, root, *extra_roots)
    ...
    def encode_snapshot(
        ...
        *extra_roots: t.Any,
        ...
        """
        Encode a snapshot (or collect one then encode) as a URL-safe
        base64 string suitable for sharing via URL query parameters.
        """
        from .snapshot import collect_state, encode_snapshot as _enc
        if snapshot is None:
            snapshot = collect_state(root, *extra_roots)
        return _enc(snapshot)

    def decode_snapshot(self, encoded: str) -> dict[str, t.Any] | None:
        """Decode a URL-safe base64 snapshot string."""
        from .snapshot import decode_snapshot as _dec
        return _dec(encoded)

    def named_snapshots(self) -> list[str]:
        """Return a sorted list of named snapshot keys in the current session."""
        from .snapshot import list_named_snapshots
        return list_named_snapshots()
    ...
    def save_named_snapshot(
        ...
        *extra_roots: t.Any,
        ...
        """
        Save (or collect and save) a snapshot under ``name``.
        Returns the snapshot that was saved.
        """
        from .snapshot import save_named_snapshot
        return save_named_snapshot(name, snapshot, root, *extra_roots)

    def load_named_snapshot(self, name: str) -> dict[str, t.Any] | None:
        """Load a named snapshot, or return None if it does not exist."""
        from .snapshot import load_named_snapshot
        return load_named_snapshot(name)

    def delete_named_snapshot(self, name: str) -> bool:
        """Delete a named snapshot; returns True if it existed."""
        from .snapshot import delete_named_snapshot
        return delete_named_snapshot(name)

    @property
    def url_snapshot(self) -> dict[str, t.Any] | None:
        """Return the decoded snapshot from the URL query parameter, if any."""
        from .snapshot import get_snapshot_from_url
        return get_snapshot_from_url()
